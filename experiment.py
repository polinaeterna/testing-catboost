...
class Experiment(object):
    ...
    def __init__(self, learning_task='classification', bst_name=None, n_estimators=5000, hyperopt_evals=50, 
                 ...
                 train_path=None, test_path=None, cd_path=None, num_class=2,
        ...
        if self.learning_task in ('classification', 'multiclassification'):
            self.metric = 'logloss'
        elif self.learning_task == 'regression':
            self.metric = 'rmse'
        else:
            raise ValueError('Task type must be "classification", "multiclassification" or "regression"')
    ...
    def split_and_preprocess(self, X_train, y_train, X_test, y_test, cat_cols, n_splits=5, random_state=0):
        ...
        for train_index, test_index in cv.split(X_train, y_train):
            ...
            dtrain = self.convert_to_dataset(train, y_train[train_index], cat_cols)
            dtest = self.convert_to_dataset(test, y_train[test_index], cat_cols)
            cv_pairs.append((dtrain, dtest))
        
        _ = self.preprocess_cat_cols(X_train, y_train, cat_cols, X_test, cc)
        dtrain = self.convert_to_dataset(X_train, y_train, cat_cols)
        dtest = self.convert_to_dataset(X_test, y_test, cat_cols)

        return cv_pairs, (dtrain, dtest), X_test
    ...
    def run_cv(self, cv_pairs, params=None, n_estimators=None, verbose=False):
        ...
        for dtrain, dtest in cv_pairs:
            _, evals_result, eval_time = self.fit(params, dtrain, dtest, n_estimators)
    ...
    def run_test(self, dtrain, dtest, X_test=None, params=None, n_estimators=None, custom_metric=None, seed=0):
        ...
        bst, evals_result, eval_time = self.fit(params, dtrain, dtest, n_estimators, seed=seed)
        #eval_time = time.time() - start_time
