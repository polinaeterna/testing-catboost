...
class CABExperiment(Experiment):
    ...
    def __init__(self, learning_task, n_estimators=5000, max_hyperopt_evals=50, 
        ...
        self.space = {
            ...
            'ctr_description': hp.choice('ctr_description', [['Borders','Counter']]),
    ...
    def preprocess_params(self, params):
        if self.learning_task == 'classification':
            params.update({'loss_function': 'Logloss', 'verbose': False, 'random_seed': 0})
        elif self.learning_task == 'multiclassification':
            params.update({'loss_function': 'MultiClass', 'classes_count': self.num_class,
                           'verbose': False, 'random_seed': 0})
        elif self.learning_task == 'regression':
            params.update({'loss_function': 'RMSE', 'verbose': False, 'random_seed': 0})
        return params
