...
class LGBExperiment(Experiment):
    ...
    def preprocess_params(self, params):
        ...
        if self.learning_task == 'classification':
            ...
            params_.update({'objective': 'multiclass', 'metric': 'multi_logloss', 'num_class': self.num_class,
                           'bagging_freq': 1, 'verbose': -1})
    ...
    def convert_to_dataset(self, data, label, cat_cols=None):
        return lgb.Dataset(data.astype(float), label)
    ...
    def fit(self, params, dtrain, dtest, n_estimators, seed=0):
        ...
        if self.learning_task == 'regression':
            results = np.power(evals_result['test']['l2'], 0.5)
        elif self.learning_task == 'classification':
            results = evals_result['test']['binary_logloss']
        elif self.learning_task == 'multiclassification':
            results = evals_result['test']['multi_logloss']
